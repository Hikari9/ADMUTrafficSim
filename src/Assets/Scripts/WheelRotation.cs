...
public class WheelRotation : MonoBehaviour {
	...
	void Start () {
		radius = new Vector3(0, GetComponent<Renderer>().bounds.extents.y, 0); // safer to get y
	}
	...
	void Update () {
		...
		Vector3 velocity = parent.parent.transform.InverseTransformDirection (parent.GetComponent<Rigidbody>().velocity); // assume there is a grandparent
		...
		transform.RotateAround (GetComponent<Renderer>().bounds.center, omega, omega.magnitude);
	}
}
